...
import { pordentroAgenda } from "@/lib/agenda";
import { cn } from "@/lib/utils";

export function PorDentroAgenda() {
  const [ativa, setAtiva] = useState(0);
  const tela = pordentroAgenda.telas[ativa];

  return (
    <section id="pordentro" className="section bg-panel">
      <div className="shell">
        <Reveal>
          <ScrambleText text={pordentroAgenda.eyebrow} className="eyebrow text-forest-400" />
        </Reveal>

        <h2 className="mt-5 max-w-3xl text-display-md md:text-display-lg">
          <SplitText text={pordentroAgenda.titulo[0]} animateOnView className="text-white" />{" "}
          <SplitText text={pordentroAgenda.titulo[1]} animateOnView delay={0.12} className="lit" />
        </h2>

        <Reveal delay={0.1}>
          <p className="mt-6 max-w-2xl text-lg text-body">{pordentroAgenda.intro}</p>
        </Reveal>

        <Reveal delay={0.15}>
          <div className="fade-x mt-12 -mx-5 overflow-x-auto px-5 md:mx-0 md:px-0">
            <div className="flex w-max gap-2 md:w-auto md:flex-wrap">
              {pordentroAgenda.telas.map((t, i) => (
                <button
                  key={t.aba}
                  type="button"
                  onClick={() => setAtiva(i)}
          ...
          <p className="mt-8 text-sm text-muted">{pordentroAgenda.rodape}</p>
        </Reveal>
      </div>
    </section>
  );
