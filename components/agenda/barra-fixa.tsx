...
import { agendaInterna, zapAgenda } from "@/lib/agenda";
import { site } from "@/lib/content";

export function BarraFixaAgenda() {
        ...
        >
          <div className="shell flex items-center justify-between gap-4 py-3.5">
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-white">{agendaInterna.nomeCompleto}</p>
              <p className="hidden truncate text-xs text-muted sm:block">{agendaInterna.tagline}</p>
            </div>
            <ButtonLink href={zapAgenda(site.whatsapp)} target="_blank" rel="noopener" className="shrink-0">
              Falar com a Nodum
            </ButtonLink>
          </div>
        </motion.div>
