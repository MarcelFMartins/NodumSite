...
import { porqueAgenda } from "@/lib/agenda";

export function PorqueAgenda() {
  return (
    <section id="porque" className="section bg-surface">
      <div className="shell">
        <Reveal>
          <ScrambleText text={porqueAgenda.eyebrow} className="eyebrow text-forest-400" />
        </Reveal>

        <h2 className="mt-5 max-w-4xl text-display-md md:text-display-lg">
          <SplitText text={porqueAgenda.titulo[0]} animateOnView className="text-white" />{" "}
          <SplitText text={porqueAgenda.titulo[1]} animateOnView delay={0.12} className="lit" />
        </h2>

        <Reveal delay={0.1}>
          <p className="mt-6 max-w-2xl text-lg text-body">{porqueAgenda.intro}</p>
        </Reveal>

        <div className="rule mt-14" />

        <Stagger className="mt-14 grid gap-5 sm:grid-cols-3">
          {porqueAgenda.cenas.map((cena, i) => (
