...
import { acessos } from "@/lib/barber";
import { cn } from "@/lib/utils";

export function Acessos() {
  const [ativo, setAtivo] = useState(0);
  const perfil = acessos.perfis[ativo];

  return (
    <section id="acessos" className="section bg-surface">
      <div className="shell">
        <Reveal>
          <ScrambleText text={acessos.eyebrow} className="eyebrow text-forest-400" />
        </Reveal>

        <h2 className="mt-5 max-w-3xl text-display-md md:text-display-lg">
          <SplitText text={acessos.titulo[0]} animateOnView className="text-white" />{" "}
          <SplitText text={acessos.titulo[1]} animateOnView delay={0.12} className="lit" />
        </h2>

        <Reveal delay={0.1}>
          <p className="mt-6 max-w-2xl text-lg text-body">{acessos.intro}</p>
        </Reveal>

        <Reveal delay={0.15}>
          <div className="mt-12 grid gap-8 lg:grid-cols-[minmax(0,18rem)_1fr] lg:gap-12">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {"nota" in perfil && perfil.nota && (
                  <p className="mt-7 border-t border-line pt-6 text-sm text-forest-400">
                    {perfil.nota}
                  </p>
                )}
              </motion.div>
