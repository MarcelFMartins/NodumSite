...
import { faqAgenda } from "@/lib/agenda";
  ...
  return (
    <section className="section bg-panel">
      <div className="shell grid gap-10 lg:grid-cols-[minmax(0,22rem)_1fr] lg:gap-16">
        <div>
          <Reveal>
            <ScrambleText text={faqAgenda.eyebrow} className="eyebrow text-forest-400" />
          </Reveal>
          <h2 className="mt-5 text-display-md">
            <SplitText text={faqAgenda.titulo} animateOnView className="text-white" />
          </h2>
        </div>

        <Reveal delay={0.1}>
          <div className="border-t border-line">
            {faqAgenda.itens.map((item, i) => (
              <Item
                key={item.p}
                p={item.p}
                r={item.r}
