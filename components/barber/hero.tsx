...
import { heroBarber, sistema } from "@/lib/barber";
import { useTelaPequena } from "@/lib/hooks";

export function HeroBarber() {
  const ref = useRef<HTMLElement>(null);
  const pequena = useTelaPequena();
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start start", "end start"] });

  // O mockup sobe mais devagar que o texto: dá profundidade sem exigir
  // uma segunda camada de imagem.
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden bg-surface pb-20 pt-32 md:pb-28 md:pt-40"
    >
      ...
      <motion.div style={{ y: yTexto, opacity: opacidade }} className="shell relative z-10">
        ...
        >
          ...
          <ScrambleText text={heroBarber.eyebrow} className="eyebrow text-forest-400" />
        </motion.div>

        <h1 className="mt-8 max-w-4xl text-[2.6rem] leading-[1.04] sm:text-[3.5rem] md:text-display-xl lg:text-[4.75rem]">
          <SplitText text={heroBarber.titulo[0]} delay={0.15} className="text-white" />{" "}
          <SplitText text={heroBarber.titulo[1]} delay={0.35} className="lit" />
        </h1>
      ...
      <div className="shell relative z-10 mt-14 grid gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        {heroBarber.provas.map((prova, i) => (
          ...
          >
            <p className="font-mono text-4xl font-bold leading-none text-forest-400">
              {prova.valor}
            </p>
