...
import { acessosBi } from "@/lib/bi";
import { cn } from "@/lib/utils";

export function AcessosBi() {
  const [ativo, setAtivo] = useState(0);
  const perfil = acessosBi.perfis[ativo];

  return (
    <section id="acessos" className="section bg-panel">
      <div className="shell">
        <Reveal>
          <ScrambleText text={acessosBi.eyebrow} className="eyebrow text-forest-400" />
        </Reveal>

        <h2 className="mt-5 max-w-3xl text-display-md md:text-display-lg">
          <SplitText text={acessosBi.titulo[0]} animateOnView className="text-white" />{" "}
          <SplitText text={acessosBi.titulo[1]} animateOnView delay={0.12} className="lit" />
        </h2>

        <Reveal delay={0.1}>
          <p className="mt-6 max-w-2xl text-lg text-body">{acessosBi.intro}</p>
        </Reveal>

        <Reveal delay={0.15}>
          <div className="mt-12 grid gap-8 lg:grid-cols-[minmax(0,16rem)_1fr] lg:gap-12">
            <div className="fade-x -mx-5 overflow-x-auto px-5 lg:mx-0 lg:overflow-visible lg:px-0">
              <div className="flex w-max gap-2 lg:w-auto lg:flex-col">
                {acessosBi.perfis.map((p, i) => (
                  <button
                    key={p.aba}
                    type="button"
                    onClick={() => setAtivo(i)}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {"nota" in perfil && perfil.nota && (
                  <p className="mt-7 border-t border-line pt-6 text-sm text-forest-400">{perfil.nota}</p>
                )}
              </motion.div>
            </AnimatePresence>
          </div>
