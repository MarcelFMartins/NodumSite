...
import { heroBi, zapBi } from "@/lib/bi";
import { site } from "@/lib/content";
import { useTelaPequena } from "@/lib/hooks";

export function HeroBi() {
    ...
    <section ref={ref} className="relative overflow-hidden bg-surface pb-20 pt-32 md:pb-28 md:pt-40">
      ...
      <motion.div style={{ y: yTexto, opacity: opacidade }} className="shell relative z-10">
        ...
        >
          ...
          <ScrambleText text={heroBi.eyebrow} className="eyebrow text-forest-400" />
        </motion.div>

        <h1 className="mt-8 max-w-4xl text-[2.5rem] leading-[1.05] sm:text-[3.25rem] md:text-display-xl lg:text-[4.4rem]">
          <SplitText text={heroBi.titulo[0]} delay={0.15} className="text-white" />{" "}
          <SplitText text={heroBi.titulo[1]} delay={0.35} className="lit" />
        </h1>
      ...
      >
        <div className="relative">
          ...
          />
          <Image
            src="/img/bi/hero-dashboard.webp"
            alt="Painel de Indicadores Financeiros do Nodum BI: cinco KPIs e o gráfico de evolução mensal de receita e lucro"
            width={1440}
            height={1010}
            priority
